  ...
  lessons: [
    {
      ...
      steps: [
        ...
        { type: "cli", title: "Як перемикати",
          intro: "<p>Під час сесії <span class=\"kbd\">Shift</span>+<span class=\"kbd\">Tab</span> по колу перемикає режими: <code>default</code> → <code>acceptEdits</code> → <code>plan</code> → … Поточний режим видно в рядку стану. З <code>auto</code> перше натискання веде в <code>default</code>.</p><p><strong>Стартовий режим:</strong> прапорець <code>--permission-mode</code> → <code>permissions.defaultMode</code> у налаштуваннях → вбудований типовий. Вбудований типовий у новіших версіях (з v2.1.228; на нативній Windows — з v2.1.233) на планах Pro, Max і Team — <code>auto</code>; на Enterprise, з API-ключем Console, у <code>claude -p</code>, на Bedrock/Vertex/Foundry і в першій сесії після встановлення чи оновлення — <code>default</code> (Manual). Тренажер стартує в Manual, щоб ти бачив запити. Хочеш, щоб агент питав, — запускай з <code>--permission-mode default</code> або постав <code>\"defaultMode\": \"default\"</code> у блок <code>permissions</code> файлу <code>~/.claude/settings.json</code>.</p>",
          commands: [
